        ...
        for event in ev:
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if dragging is None:
                    ...
                    for v in graph.vertices:
                        if (v.x - new_v.x) ** 2 + (v.y - new_v.y) ** 2 < graph.EDGE_CREATION_RADIUS ** 2:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.key.key_code("z"):
                    v = graph.vertices.pop()
                    graph.edges = [
                        e for e in graph.edges if e.start != v and e.end != v]

        for _ in range(graph.SUBSTEPS):
            graph._step(16)
        graph.draw(screen)
        clock.tick(60)
